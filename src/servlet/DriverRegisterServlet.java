	...
	protected void doPost(HttpServletRequest req, HttpServletResponse resp)
		...
		if (password.equals(password1)) {
			...
			try {
				...
				out.print("<script>alert('信息有误，请重新注册！'); window.location='driver_register.jsp' </script>");
				return;
			}
			...
			if (affect == 1) {
				out.print("<script>alert('注册成功！'); window.location='driver_register.jsp' </script>");
			}else if (affect == -1) {
				out.print("<script>alert('注册失败，请重新注册！'); window.location='driver_register.jsp' </script>");
			}
			out.flush();
			out.close();
		}
